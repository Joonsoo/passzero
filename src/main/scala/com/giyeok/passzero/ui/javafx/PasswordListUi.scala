...
import com.giyeok.passzero.{PasswordManager, Session}
...
class PasswordListController {
    @FXML
    var globalHamburger: JFXHamburger = _
    @FXML
...
class PasswordListUi(mainUi: JavaFxUI, session: Session) extends JavaFxUI.View {
    ...
    override def viewRoot(): Parent = {
        ...
        controller.directoryComboBox.setCellFactory(new LabelListCell({_.name}))
        controller.directoryComboBox.setConverter(new OnewayStringConverter({_.name}))

        // TODO 아이콘으로 변경
        controller.sheetList.setCellFactory(new LabelListCell({_.name}))
        ...
        controller.sheetListSortScheme.setCellFactory(new LabelListCell({_.name}))
        controller.sheetListSortScheme.setConverter(new OnewayStringConverter({_.name}))
                ...
                Platform.runLater { () =>
                    passwordManager.directory.directoryList() map { directories =>
                        directories foreach { directoryPair =>
                            ...
                            if ((configOpt map {_.defaultDirectory}) contains Some(directoryId)) {
                                Platform.runLater { () =>
    ...
    private def createSheetDetailGrid(directoryInfo: DirectoryInfo, sheetId: SheetId, sheetInfo: SheetInfo, sheetDetail: Option[SheetDetail]): GridPane = {
        ...
        passwordStore.sheet(sheetId) foreach { sheetDetail: Option[(SheetInfo, Option[SheetDetail])] =>
            Platform.runLater { () =>
                ...
                if (selected.size() == 1 && selected.get(0).id == sheetId) {
                    ...
                    controller.sheetContentHolder.setContent(createSheetDetailGrid(directoryInfo, sheetId, sheetInfo, sheetDetail flatMap {_._2}))
                }
            }
        }
    }
    ...
    def editSheetDetail(directoryInfo: DirectoryInfo, sheetId: SheetId, sheetInfo: SheetInfo, sheetDetail: Option[(SheetInfo, Option[SheetDetail])]): Unit = {
        ...
        controller.sheetContentHolder.setContent(???)
    }
}
